...
type OrdDataSpecification struct {
	...
	Subagent       OrdSubagent `json:"subagent"`        //Данные о подрядчике в формате JSON (ord_subagent).
...
type CreateClientsSpecification struct {
	...
	OrdData  OrdDataSpecification `json:"ord_data"`  //Данные оператора рекламных данных в формате JSON (ord_data_specification).
}
